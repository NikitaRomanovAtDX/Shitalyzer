...
{
    /// <summary>
    /// SHIT0002: flags calls to <see cref="string"/> method overloads that exist in .NET Core /
    /// .NET Standard 2.1 but not in .NET Framework 4.7.2. The project multi-targets both, so these
    /// calls fail to compile on the .NET Framework leg.
    /// </summary>
    ...
    public sealed class NetFrameworkIncompatibleMethodAnalyzer : DiagnosticAnalyzer
    {
        ...
        private static readonly DiagnosticDescriptor Rule = new DiagnosticDescriptor(
            ...
            title: "String method overload is missing in .NET Framework 4.7.2",
            messageFormat: "'string.{0}' does not exist in .NET Framework 4.7.2; use a compatible alternative",
            category: Categories.Compatibility,
            defaultSeverity: DiagnosticSeverity.Warning,
            isEnabledByDefault: true,
            description: "The code multi-targets .NET Core and .NET Framework 4.7.2. This string overload only exists on .NET Core / .NET Standard 2.1 and will not compile on .NET Framework.");
            ...
            switch (method.Name)
            {
                case "StartsWith":
                    if (IsSingle(parameters, SpecialType.System_Char))
                        return (true, StartsWithChar, "StartsWith(char)");
                    break;

                case "EndsWith":
                    if (IsSingle(parameters, SpecialType.System_Char))
                        return (true, EndsWithChar, "EndsWith(char)");
                    break;

                case "Contains":
                    if (IsSingle(parameters, SpecialType.System_Char))
                        return (true, ContainsChar, "Contains(char)");
                    if (parameters.Length == 2 && IsStringComparison(parameters[1].Type))
                    {
                        ...
                        return (true, null, $"Contains({first}, StringComparison)");
                    }
                    break;

                case "GetHashCode":
                    if (parameters.Length == 1 && IsStringComparison(parameters[0].Type))
                        return (true, null, "GetHashCode(StringComparison)");
                    break;
                ...
                case "Split":
                    ...
                    {
                        var first = parameters[0].Type.SpecialType == SpecialType.System_Char ? "char" : "string";
                        return (true, null, $"Split({first}, StringSplitOptions)");
                    }
                    break;
            }
        ...
        private static bool IsStringComparison(ITypeSymbol type) =>
            type.Name == "StringComparison" && type.ContainingNamespace?.ToDisplayString() == "System";
    }
}
